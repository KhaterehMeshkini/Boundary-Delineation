        ...
        for layer in resnet_layers:
            if isinstance(layer, supported_layers):
                if isinstance(layer, nn.Conv2d):
                    ...
                    expanded_weights = original_weights.repeat(1, 4, 1, 1)[:, :, :, :]  # Shape: (out_channels, 10, H, W)
...
class DynamicAttentionUNet3D(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        self.decoder1 = nn.Sequential(
            nn.ConvTranspose3d(256, 128, kernel_size=(2, 2, 2), stride=(2, 2, 2)),   # 64x64 → 128x128
            nn.ReLU(inplace=True),
        ...
        self.collapse1 = nn.Conv3d(64, 32, kernel_size=(3, 1, 1))  
        self.collapse2 = nn.Conv3d(32, 16, kernel_size=(3, 1, 1))  
        self.collapse3 = nn.Conv3d(16, 8, kernel_size=(3, 1, 1))  
        self.collapse4 = nn.Conv2d(16, num_classes, kernel_size=(1, 1))
        # self.final_upsample = nn.Sequential(
        #     nn.ConvTranspose2d(16, 8, kernel_size=1, stride=1),  # 128x128 → 256x256
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(8, num_classes, kernel_size=1)            # Final classification layer
        # )
    ...
    def forward(self, data):
        ...
        B, C, D, H, W = x.shape
        x_out = x.view(1, -1, H, W)              
        output = self.collapse4(x_out)
